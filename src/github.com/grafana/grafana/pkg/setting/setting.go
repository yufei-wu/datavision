...
func loadConfiguration(args *CommandLineArgs) error {
	...
	if _, err := os.Stat(defaultConfigFile); os.IsNotExist(err) {
		fmt.Println("datavision-server Init Failed: Could not find config defaults, make sure homepath command line parameter is set or working directory is homepath")
		os.Exit(1)
	}
