...
func GetDashboard(c *m.ReqContext) Response {
	...
	if dash.FolderId > 0 {
		...
		if err := bus.Dispatch(&query); err != nil {
			return Error(500, "仪表盘文件夹不能被读取", err)
		}
...
func DeleteDashboard(c *m.ReqContext) Response {
	...
	return JSON(200, util.DynMap{
		"title":   dash.Title,
		"message": fmt.Sprintf("仪表盘 %s 已删除", dash.Title),
	})
}
...
func GetHomeDashboard(c *m.ReqContext) Response {
	...
	if err != nil {
		return Error(500, "加载首页仪表盘失败", err)
	}
