...
export class ManageDashboardsCtrl {
  ...
  delete() {
    ...
    if (folderCount > 0 && dashCount > 0) {
      text += `已选择的文件夹${folderCount === 1 ? '' : 's'} 和面板${dashCount === 1 ? '' : 's'}?`;
      text2 = `当前选择的文件夹下的所有面板${folderCount === 1 ? '' : 's'} 都将会被删除`;
    } else if (folderCount > 0) {
      text += `已选择的文件夹${folderCount === 1 ? '' : 's'} 和它包含的所有面板?`;
    } else {
      text += `已选择的面板${dashCount === 1 ? '' : 's'}?`;
    }
