...
var (
	...
	buildNumber           int      = 0
	binaries              []string = []string{"datavision-server", "datavision-cli"}
)
...
func main() {
	...
	for _, cmd := range flag.Args() {
		...
		case "build-srv":
			clean()
			build("datavision-server", "./pkg/cmd/datavision-server", []string{})

		case "build-cli":
			clean()
			build("datavision-cli", "./pkg/cmd/datavision-cli", []string{})
...
func makeLatestDistCopies() {
	...
	latestMapping := map[string]string{
		".deb":    "dist/datavision_latest_amd64.deb",
		".rpm":    "dist/datavision-latest-1.x86_64.rpm",
		".tar.gz": "dist/datavision-latest.linux-x64.tar.gz",
	}
...
func createDebPackages() {
	createPackage(linuxPackageOptions{
		packageType:            "deb",
		homeDir:                "/usr/share/datavision",
		binPath:                "/usr/sbin",
		configDir:              "/etc/datavision",
		etcDefaultPath:         "/etc/default",
		etcDefaultFilePath:     "/etc/default/datavision-server",
		initdScriptFilePath:    "/etc/init.d/datavision-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/datavision-server.service",

		postinstSrc:    "packaging/deb/control/postinst",
		initdScriptSrc: "packaging/deb/init.d/datavision-server",
		defaultFileSrc: "packaging/deb/default/datavision-server",
		systemdFileSrc: "packaging/deb/systemd/datavision-server.service",
...
func createRpmPackages() {
	createPackage(linuxPackageOptions{
		...
		etcDefaultFilePath:     "/etc/sysconfig/datavision-server",
		initdScriptFilePath:    "/etc/init.d/datavision-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/datavision-server.service",

		postinstSrc:    "packaging/rpm/control/postinst",
		initdScriptSrc: "packaging/rpm/init.d/datavision-server",
		defaultFileSrc: "packaging/rpm/sysconfig/datavision-server",
		systemdFileSrc: "packaging/rpm/systemd/datavision-server.service",
...
func createPackage(options linuxPackageOptions) {
	packageRoot, _ := ioutil.TempDir("", "datavision-linux-pack")
	...
	args := []string{
		"-s", "dir",
		"--description", "DataVision",
		"-C", packageRoot,
		"--vendor", "lingzhi",
		"--url", "https://www.lzgk.com",
		"--license", "Business License",
		"--maintainer", "contact@datavision.com",
...
func setup() {
	...
	runPrint("go", "install", "-v", "./pkg/cmd/datavision-server")
}
