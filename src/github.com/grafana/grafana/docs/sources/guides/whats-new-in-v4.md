...
the right side of the graph. The rules will continually be evaluated by datavision-server and
...
There are no breaking changes. Old dashboards and features should work the same. datavision-server will automatically upgrade its db
...
You can update plugins using datavision-cli
