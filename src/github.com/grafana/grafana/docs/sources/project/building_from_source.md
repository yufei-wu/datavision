...
go run build.go build              # (or 'go build ./pkg/cmd/datavision-server')
```
...
```bash
./bin/datavision-server
```
If you built the binary with `go run build.go build`, run `./bin/datavision-server`
