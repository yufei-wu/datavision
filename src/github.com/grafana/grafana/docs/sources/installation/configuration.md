...
通过环境变量或者 `.ini` 来配置DataVision提供的一系列变量

## 配置文件路径

- 默认配置为 `$WORKING_DIR/conf/defaults.ini`
- 自定义配置为 `$WORKING_DIR/conf/custom.ini`
- 可以通过设置 `--config` 参数来覆盖配置文件地址

> **注意.** 如果您是通过 `deb` 或者 `rpm` 包来安装DataVision
> , 配置文件会存放在
> `/etc/datavision/datavision.ini`. 这个路径可以通过使用 `--config` 参数在
> init.d 脚本进行指定 .
...
设置 datavision-server 的实例名称. 默认为: `${HOSTNAME}`

## [paths]

### data

系统sqlite3 数据库 (如果使用), file based
sessions (如果使用), 以及其他数据的路径. 
...
包含平台启动时应用的文件的文件夹 [provisioning](/administration/provisioning) 

## [server]

### http_addr

平台绑定的IP地址，如果不指定，则绑定所有的网卡

### http_port

平台HTTP端口，默认为 `3000`. 如果要使用 80 端口，您需要设置 DataVision 二进制权限，如下:

```bash
$ sudo setcap 'cap_net_bind_service=+ep' /usr/sbin/datavision-server
```
...
另外，您也可以通过设置一个web服务器（比如 Nginx 或者 Apache） 在DataVision前面来进行请求代理。
